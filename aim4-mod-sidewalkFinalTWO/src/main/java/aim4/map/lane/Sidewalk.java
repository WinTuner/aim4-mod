...
public class Sidewalk {
  ...
  /**
   * Return the true centerline of the sidewalk (midway between inner and
   * outer edges). This accounts for the fact that the sidewalk polygon is
   * built using an outer offset of `getWidth()` (which is twice the internal
   * `width` parameter). The true center offset should therefore be
   * `getWidth()/2`.
   */
  public Shape getTrueCenterLine() {
    try {
      if (lane == null) return null;
      Shape innerShape = (!lane.hasRightNeighbor()) ? lane.rightBorder() : lane.leftBorder();
      Line2D innerLine = null;
      ...
      Point2D mid = new Point2D.Double((x1 + x2) / 2.0, (y1 + y2) / 2.0);
      double testDist = Math.max(0.5, Math.abs(getWidth()) / 4.0);
      Point2D testOut = new Point2D.Double(mid.getX() + nx * testDist,
          mid.getY() + ny * testDist);
      boolean normalPointsOutside = false;
      try {
      ...
      // outer offset used for polygon is abs(getWidth()), so midpoint offset is half of that
      double offset = Math.abs(getWidth()) / 2.0;
      double cx1 = x1 + nx * offset;
      double cy1 = y1 + ny * offset;
      double cx2 = x2 + nx * offset;
      double cy2 = y2 + ny * offset;
